...
public class EditorObjExporter : ScriptableObject
{
	...
	[MenuItem ("RuntimeExporter/Export/Export all MeshFilters in selection to separate OBJs")]
	...
	[MenuItem ("RuntimeExporter/Export/Export whole selection to single OBJ")]
	...
	[MenuItem ("RuntimeExporter/Export/Export each selected to single OBJ")]
	...
	*/
	...
	[MenuItem ("RuntimeExporter/Export/Test Export whole selection to single OBJ")]
	static void TestExportWholeSelectionToSingle()
	{
		if (!CreateTargetFolder())
			return;
		...
		if (exportedObjects > 0)
		{
			...
			Debug.Log ("Folder name " + targetFolder);
			Debug.Log ("Selection name " + filename);
			MeshesToFile(mf, targetFolder, filename);
	...
	//Testing
	[MenuItem ("RuntimeExporter/Export/Test Export each selected to single OBJ")]
	static void TestExportEachSelectionToSingle()
	{
		if (!CreateTargetFolder())
			return;
		...
		for (int i = 0; i < selection.Length; i++)
		{
			...
			//testing
			string filename = EditorSceneManager.GetActiveScene().name + "_" + i.ToString();

			Debug.Log ("Folder name " + targetFolder);
			Debug.Log ("Selection name " + selection[i].name.ToString());
